...
export const calculateOrderBudget = async (tokens: string[]): Promise<number> => {
  const secret = process.env.BINANCE_API_SECRET;
  const apiKey = process.env.BINANCE_API_KEY;

  if (!secret || !apiKey) {
    console.log('[calculateOrderBudget] Faltan credenciales de Binance.');
    return 0;
  }
  ...
  try {
    ...
    if (!usdtEntry) {
      console.log('[calculateOrderBudget] No se encontró saldo para USDT.');
      return 0;
    }
    ...
    console.log('[calculateOrderBudget] No se pudo obtener el saldo.');
    console.log((error as any).response?.data || (error as Error).message);
    return 0;
  }

  const items = tokens.length;
  console.log(`[calculateOrderBudget] Elementos en la lista (${items}):`, tokens);
